...
async function fetchJson(url, options = {}) {
  ...
  try {
    ...
    throw new Error(`Fetch failed for ${url}: ${error?.message || "network error"}`);
  }
...
function predictWithTraining(sample, training, options = {}) {
  ...
  return {
    ...
    neighborReliability,
    features: {
      distanceToStrike: sample.features?.distanceToStrike ?? null,
      priceRet24: sample.features?.priceRet24 ?? null,
    },
  };
}
...
export async function buildPolymarketBtcUpDownResearch({
  ...
  return {
    meta: {
      source: regime.meta?.source || source,
...
function compactResultRow(row) {
  return {
    ...
    prediction: row.prediction ? {
      ...
      weights: row.prediction.weights || null,
      baseWeight: row.prediction.baseWeight || null,
      weightSum: row.prediction.weightSum || null,
      trendKey: row.prediction.trendKey || null,
      stateReliability: row.prediction.stateReliability || null,
      trendReliability: row.prediction.trendReliability || null,
      neighborReliability: row.prediction.neighborReliability || null,
      features: row.prediction.features || null,
    } : null,
