...
function hoursToSteps(hours, interval) {
  if (interval === "30m") return Math.round(hours * 2);
  if (interval === "4h") return Math.round(hours / 4);
  if (interval === "1d") return Math.round(hours / 24);
  return hours; // default to 1h
}
  ...
  return {
    ...
    meta: {
      ...
      kSelection: {
        ...
        note: "Fit K controls the HMM state count. Auto uses highest silhouette; elbow, AIC, BIC, and Interpretability are diagnostics.",
      },
      logLik: hmm.logLik,
