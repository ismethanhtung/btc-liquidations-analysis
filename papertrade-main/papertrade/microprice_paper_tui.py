"""
TUI dashboard for `microprice_paper_trader.py` (5m markets).

Run:
    python -m papertrade.microprice_paper_tui --coin btc
    python papertrade/microprice_paper_tui.py --coin btc
"""
...
def render(s: DashState, coin: str, rows_avail: int = 60):
    ...
    # Header
    _w(f"{_BORDER_TOP}\n")
    title = f"  PAPER TRADER ({coin.upper()}) — full-taker GTC bump-N + adverse-move guard  "
    ...
    # Market header — current 5m market (rolls every bucket like microprice_mm_5m)
    ...
    # Current 5-min session — MTM + fees paid on this session's fills
    sess_fees_str = (
...
def main():
    p = argparse.ArgumentParser(
        description="TUI dashboard for microprice_paper_trader.py"
    )
    ...
    script = Path(__file__).with_name("microprice_paper_trader.py")
