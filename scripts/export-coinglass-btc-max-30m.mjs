...
async function fetchPage(endTime) {
  ...
  for (let retry = 0; retry < 8; retry += 1) {
    ...
    if (code === '429') {
      await sleep(Math.min(60000, 1500 * (retry + 1)));
      continue;
    }
...
while (calls < 1500) {
  ...
  if (prevMinTs !== null && minTs === prevMinTs) repeatedOldestCount += 1;
  else repeatedOldestCount = 0;
  prevMinTs = minTs;
...
const csvPath = path.join(process.cwd(), 'data', 'coinglass_BTC_liquidation_max_30m.csv');
const metaPath = path.join(process.cwd(), 'data', 'coinglass_BTC_liquidation_max_30m.meta.json');
