...
function buildFeatureRows(rows, interval) {
  const barsPerDay = intervalBarsPerDay(interval);
...
function buildPcaProjection(matrix, timeline, chosenK) {
  ...
  return {
    points,
    explained: [Math.max(0, pc1.value / totalVar), Math.max(0, pc2.value / totalVar)],
  };
}
...
function pickBestK(matrix, maxK = 6) {
  ...
  for (let k = 2; k <= upper; k += 1) {
    ...
    const score = sil + 0.35 * drop;
    candidates.push({ k, inertia: fit.inertia, silhouette: sil, elbowDrop: drop, score, labels: fit.labels, centroids: fit.centroids });
    prevInertia = fit.inertia;
  }
  const bestScore = [...candidates].sort((a, b) => b.score - a.score)[0] || null;
  ...
  return { best: bestSilhouette, bestScore, bestSilhouette, bestElbow, candidates };
}
...
function describeRegime(stateSummary, state) {
  ...
  return {
    label: `State ${state + 1}`,
    strategy: "No strategy label assigned",
    tone: "neutral",
    scores: {
      cascadeScore,
      trendScore,
...
function inferIntervalFromRows(rows) {
  ...
  if (step <= 30 * MS.min + 1) return "30m";
  if (step >= 4 * MS.hour - 1 && step <= 4 * MS.hour + 1) return "4h";
  if (step >= MS.day - 1) return "1d";
  return "1h";
}
...
function buildRegimeResultFromMerged({
  ...
  const kSelection = pickBestK(matrix, maxK);
  ...
  return {
    meta: {
      ...
      kSelection: {
        method: Number.isFinite(requestedK) && requestedK >= 2 ? "manual" : "silhouette",
        fitK: chosenK,
        ...
        note: "Fit K controls the HMM state count. Auto uses highest silhouette; elbow and combined score are diagnostics.",
      },
      logLik: hmm.logLik,
    ...
    chart: {
      ...
      markers: timeline
        ...
        .map((row) => ({
          time: row.time,
          position: row.state === latest?.state ? "aboveBar" : "belowBar",
          color: row.color,
          shape: "circle",
          text: "",
        })),
