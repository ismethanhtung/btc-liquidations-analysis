...
function gradeProbability(p) {
    ...
    if (v >= 0.45)
        return { label: "Cân bằng", tone: "neutral", note: "45-54%" };
...
function medianStepMs(candles, rows) {
    const src =
        candles.length > 10
            ? candles
            : rows.map((r) => ({ time: Math.floor(r.ts / 1000) }));
...
function buildEventDetail(
    ...
    if (!rows.length || !candles.length || !Number.isFinite(eventTs))
        return null;
    ...
    const zWindowBars = Math.max(
        10,
        Math.round((opts.zWindowHours || 168) * barsPerHour),
    );
    const rowStart = 0;
    ...
    const pointAt = (idx) => {
        ...
        const candleIdx = nearestIndexByTs(
            candles,
            r.ts,
            (c) => Number(c.time) * 1000,
        );
    ...
    const entryIdx = Number.isFinite(entryMs)
        ? nearestIndexByTs(rows, entryMs, (r) => r.ts)
        : -1;
    const exitIdx = Number.isFinite(exitMs)
        ? nearestIndexByTs(rows, exitMs, (r) => r.ts)
        : -1;
    const entryPoint = pointAt(entryIdx);
    const exitPoint = pointAt(exitIdx);
    const priceByTime = new Map(
        priceCandles.map((c) => [Number(c.time), Number(c.close)]),
    );
...
function buildCascadeAnalysis(rows, candles, opts) {
    ...
    const merged = rows
        .map((r) => ({ ...r, candle: priceByTs.get(r.ts) }))
        .filter((x) => x.candle);
    ...
    for (let i = 0; i < merged.length; i += 1) {
        ...
        const isCascade =
            m.totalUsd >= threshold &&
            longShare >= (opts.minLongShare || 0.65) &&
            z >= (opts.zMin || 1.5);
...
export default function AnalysisDashboard({
    ...
    useEffect(() => {
        let mounted = true;
        async function run() {
            if (!stats?.summary?.rangeStart || !stats?.summary?.rangeEnd)
                return;
            try {
                setChartErr(null);
                ...
                const data = await fetchBinanceCandles(
                    stats.summary.rangeStart,
                    stats.summary.rangeEnd,
                    interval,
                );
                if (!mounted) return;
                setCandles(data);
    ...
    const hoverMonthCoverage =
        stats.byMonth.find((m) => m.month === hoverMonth) || null;
    const cascade = useMemo(
        () => buildCascadeAnalysis(rows, candles, cascadeOptions || {}),
        [rows, candles, cascadeOptions],
    );
    const isAnalysis2 = mode === "analysis2";
    const isAnalysis3 = mode === "analysis3";
    const basePath = isAnalysis2
        ? "/analysis-2"
        : isAnalysis3
          ? "/analysis-3"
          : "/analysis";
    ...
    const allFutureRelatedSecSet = useMemo(
        () =>
            new Set(
                (futureInferResult?.regime?.topNeighbors || []).map((e) =>
                    Math.floor(Number(e.ts || 0) / 1000),
                ),
            ),
        [futureInferResult],
    );
    ...
    const detailPriceMarkers = useMemo(() => {
        ...
        return cascade.events
            .filter((e) =>
                selectedEventDetail.priceByTime.has(Math.floor(e.ts / 1000)),
            )
            .map((e) => {
    ...
    const detailDataMarkers = useMemo(() => {
        ...
        return cascade.events
            .filter((e) =>
                selectedEventDetail.dataByTime.has(Math.floor(e.ts / 1000)),
            )
            .map((e) => {
                const sec = Math.floor(e.ts / 1000);
                const wl = Number.isFinite(e.ret) ? (e.win ? "W" : "L") : "N";
    ...
    useEffect(() => {
        ...
        if (
            !cascade.events.some((e) => e.ts === selectedEventTs) &&
            !allFutureRelatedSecSet.has(Math.floor(selectedEventTs / 1000))
        ) {
    ...
    useEffect(() => {
        ...
        if (
            Number.isFinite(selectedEventDetail.eventClose) &&
            selectedEventDetail.priceCandles.length
        ) {
            const first = selectedEventDetail.priceCandles[0].time;
            const last =
                selectedEventDetail.priceCandles[
                    selectedEventDetail.priceCandles.length - 1
                ].time;
        ...
        const onPriceCrosshairMove = (param) => {
            ...
            const dataIdx = nearestIndexByTs(
                selectedEventDetail.dataSeries,
                param.time * 1000,
                (d) => Number(d.time) * 1000,
            );
            if (dataIdx < 0) return;
            ...
            if (
                ...
                !detailDataChartApiRef.current ||
                !detailDataSeriesApiRef.current
            )
                return;
            syncCrosshairRef.current = true;
            try {
                detailDataChartApiRef.current.setCrosshairPosition(
                    mapped.totalUsd,
                    time,
                    detailDataSeriesApiRef.current,
                );
        ...
        return () => {
            chart
                .timeScale()
                .unsubscribeVisibleLogicalRangeChange(onPriceRange);
    ...
    useEffect(() => {
        ...
        totalSeries.setData(
            selectedEventDetail.dataSeries.map((d) => ({
                time: d.time,
                value: d.totalUsd,
                color: d.isEvent
                    ? "#dc2626"
                    : allCascadeEventSecSet.has(d.time)
                      ? "#f59e0b"
                      : "#334155",
            })),
        );
        ...
        longShareSeries.setData(
            selectedEventDetail.dataSeries.map((d) => ({
                time: d.time,
                value: d.longSharePct,
            })),
        );
        ...
        const onDataRange = (range) => {
            if (
                !range ||
                syncRangeRef.current ||
                !detailPriceChartApiRef.current
            )
                return;
            syncRangeRef.current = true;
            try {
        ...
        const onDataCrosshairMove = (param) => {
            ...
            if (typeof param.time !== "number") return;
            const priceIdx = nearestIndexByTs(
                selectedEventDetail.priceCandles,
                param.time * 1000,
                (c) => Number(c.time) * 1000,
            );
            ...
            if (
                syncCrosshairRef.current ||
                !detailPriceChartApiRef.current ||
                !detailPriceSeriesApiRef.current
            )
                return;
            ...
            syncCrosshairRef.current = true;
            try {
                detailPriceChartApiRef.current.setCrosshairPosition(
                    close,
                    time,
                    detailPriceSeriesApiRef.current,
                );
    ...
    async function runOptimize() {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(
                    json?.error || `Optimize failed: ${res.status}`,
                );
            setOptResult(json);
        } catch (e) {
    ...
    async function runFutureInfer() {
        ...
        try {
                ...
                body: JSON.stringify({
                    ...
                    baseline: {
                        entryDelayHours: Number(
                            cascadeOptions?.entryDelayHours ?? 1,
                        ),
            ...
            if (!res.ok)
                throw new Error(
                    json?.error || `Future inference failed: ${res.status}`,
                );
    ...
    const detailHoverSnapshot = useMemo(() => {
        if (!selectedEventDetail || !Number.isFinite(detailHoverTs))
            return null;
        const dataIdx = nearestIndexByTs(
            selectedEventDetail.dataSeries,
            detailHoverTs,
        ...
        const dataPoint =
            dataIdx >= 0 ? selectedEventDetail.dataSeries[dataIdx] : null;
        const pricePoint =
            priceIdx >= 0 ? selectedEventDetail.priceCandles[priceIdx] : null;
    ...
    const regimeMeanRevertGrade = gradeProbability(
        futureInferResult?.regime?.probs?.mean_revert || 0,
    );
    const regimeTrendDownGrade = gradeProbability(
        futureInferResult?.regime?.probs?.trend_down || 0,
    );
        ...
        <div className="panel-shell space-y-4">
            <div className="panel-header px-5 py-4">
                <h1 className="text-[18px] font-semibold">
                    {isAnalysis2
                        ? "Phân tích 2 (Tối ưu tham số)"
                        : isAnalysis3
                          ? "Phân tích 3 (Future Inference theo Event)"
                          : "Phân tích Liquidation (Dễ hiểu cho người mới)"}
                </h1>
                <p className="text-[12px] text-[var(--text-muted)]">
                    Di chuột trên chart nến BTC để đồng bộ các số liệu bên dưới
                    theo mốc thời gian.
                </p>
            </div>
            ...
            <div className="px-5 pt-4">
                <div className="flex flex-wrap gap-2">
                    {datasets.map((name) => (
                        <a
                            key={name}
                            href={`${basePath}?dataset=${encodeURIComponent(name)}`}
                            className={`badge ${name === chosen ? "" : "opacity-70"}`}
                        >
                            {name}
                        </a>
                    ))}
                ...
                <div className="border border-[var(--border-color)] p-3">
                    <div className="flex items-center justify-between mb-2">
                        <p className="text-[12px] font-semibold">
                            BTCUSDT Candlestick (Binance, TradingView)
                        </p>
                        <p className="text-[11px] text-[var(--text-muted)]">
                            Cursor: {fmtTs(hoverIso)}
                        </p>
                    </div>
                    <div ref={chartRef} className="w-full h-[660px]" />
                    {chartErr && (
                        <p className="text-[11px] text-[var(--danger-text)] mt-2">
                            Lỗi tải dữ liệu chart: {chartErr}
                        </p>
                    )}
                </div>
            </div>
            ...
            {isAnalysis2 ? (
                <div className="px-5">
                    <div className="border border-[var(--border-color)] p-3 space-y-3">
                        <p className="text-[12px] font-semibold">
                            Tối ưu tham số (quét tổ hợp)
                        </p>
                        <div className="grid gap-2 lg:grid-cols-6">
                            <InputSmall
                                label="Q min"
                                value={optRanges.qMin}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, qMin: v }))
                                }
                            />
                            <InputSmall
                                label="Q max"
                                value={optRanges.qMax}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, qMax: v }))
                                }
                            />
                            <InputSmall
                                label="Q step"
                                value={optRanges.qStep}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, qStep: v }))
                                }
                            />
                            <InputSmall
                                label="Long min"
                                value={optRanges.longMin}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, longMin: v }))
                                }
                            />
                            <InputSmall
                                label="Long max"
                                value={optRanges.longMax}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, longMax: v }))
                                }
                            />
                            <InputSmall
                                label="Long step"
                                value={optRanges.longStep}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, longStep: v }))
                                }
                            />
                            <InputSmall
                                label="Z min"
                                value={optRanges.zMin}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, zMin: v }))
                                }
                            />
                            <InputSmall
                                label="Z max"
                                value={optRanges.zMax}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, zMax: v }))
                                }
                            />
                            <InputSmall
                                label="Z step"
                                value={optRanges.zStep}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, zStep: v }))
                                }
                            />
                            <InputSmall
                                label="Delay min (giờ)"
                                value={optRanges.delayMin}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, delayMin: v }))
                                }
                            />
                            <InputSmall
                                label="Delay max (giờ)"
                                value={optRanges.delayMax}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, delayMax: v }))
                                }
                            />
                            <InputSmall
                                label="Delay step (giờ)"
                                value={optRanges.delayStep}
                                onChange={(v) =>
                                    setOptRanges((s) => ({
                                        ...s,
                                        delayStep: v,
                            ...
                            <InputSmall
                                label="Hold min (giờ)"
                                value={optRanges.holdMin}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, holdMin: v }))
                                }
                            />
                            <InputSmall
                                label="Hold max (giờ)"
                                value={optRanges.holdMax}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, holdMax: v }))
                                }
                            />
                            <InputSmall
                                label="Hold step (giờ)"
                                value={optRanges.holdStep}
                                onChange={(v) =>
                                    setOptRanges((s) => ({ ...s, holdStep: v }))
                                }
                            />
                            <InputSmall
                                label="Z window (h)"
                                value={optRanges.zWindowHours}
                                onChange={(v) =>
                                    setOptRanges((s) => ({
                                        ...s,
                                        zWindowHours: v,
                        ...
                        <button
                            onClick={runOptimize}
                            className="border border-[var(--border-color)] px-3 py-2 text-[11px] font-semibold"
                        >
                            {optLoading ? "Đang tối ưu..." : "Tối ưu ngay"}
                        </button>
                        <p className="text-[11px] text-[var(--text-muted)]">
                            Tất cả Delay/Hold đều tính theo giờ. Bạn có thể nhập
                            step thập phân (ví dụ 0.5 giờ) để quét chi tiết cho
                            dataset 30m. Mặc định quét Quantile từ 0.90 đến
                            0.99.
                        </p>
                        {optError ? (
                        ...
                        {optResult ? (
                            <div className="max-h-[320px] overflow-auto thin-scrollbar">
                                <table className="w-full text-[11px]">
                                    <thead>
                                        <tr className="border-b border-[var(--border-color)] text-left text-[var(--text-muted)]">
                                            ...
                                            <th className="py-2">AvgRet</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {optResult.top.map((x, i) => (
                                            <tr
                                                key={`${x.q}-${x.minLongShare}-${x.zMin}-${x.entryDelayHours}-${x.holdHours}-${i}`}
                                                className="border-b border-[var(--border-color)]/70"
                                            >
                                                <td className="py-2">{x.q}</td>
                                                <td className="py-2">
                                                    {x.minLongShare}
                                                </td>
                                                <td className="py-2">
                                ...
                                <p className="text-[10px] text-[var(--text-muted)] mt-2">
                                    Đã test {fmtInt(optResult.tested)} tổ hợp.
                                </p>
                            </div>
                        ) : null}
                    </div>
            ...
            {isAnalysis3 ? (
                <div className="px-5">
                    <div className="border border-[var(--border-color)] p-3 space-y-3">
                        <p className="text-[12px] font-semibold">
                            Future Inference (Snapshot hiện tại -&gt; so với
                            event quá khứ -&gt; logits/softmax -&gt; chọn
                            Delay/Hold)
                        </p>
                        <div className="grid gap-2 lg:grid-cols-6">
                            <InputSmall
                                label="Delay min (giờ)"
                                value={inferRanges.delayMin}
                                onChange={(v) =>
                                    setInferRanges((s) => ({
                                        ...s,
                                        delayMin: v,
                                ...
                                info="Độ trễ vào lệnh nhỏ nhất hệ thống sẽ thử. 0 nghĩa là vào ngay khi event xuất hiện."
                            />
                            <InputSmall
                                label="Delay max (giờ)"
                                value={inferRanges.delayMax}
                                onChange={(v) =>
                                    setInferRanges((s) => ({
                                        ...s,
                                        delayMax: v,
                                ...
                                info="Độ trễ vào lệnh lớn nhất hệ thống sẽ thử khi chấm action."
                            />
                            <InputSmall
                                label="Delay step (giờ)"
                                value={inferRanges.delayStep}
                                onChange={(v) =>
                                    setInferRanges((s) => ({
                                        ...s,
                                        delayStep: v,
                                ...
                                info="Bước nhảy của delay. Ví dụ min=0, max=2, step=0.5 sẽ tạo 0h, 0.5h, 1h, 1.5h, 2h."
                            />
                            <InputSmall
                                label="Hold min (giờ)"
                                value={inferRanges.holdMin}
                                onChange={(v) =>
                                    setInferRanges((s) => ({
                                        ...s,
                                        holdMin: v,
                                ...
                                info="Thời gian giữ lệnh ngắn nhất hệ thống xét."
                            />
                            <InputSmall
                                label="Hold max (giờ)"
                                value={inferRanges.holdMax}
                                onChange={(v) =>
                                    setInferRanges((s) => ({
                                        ...s,
                                        holdMax: v,
                                ...
                                info="Thời gian giữ lệnh dài nhất hệ thống xét."
                            />
                            <InputSmall
                                label="Hold step (giờ)"
                                value={inferRanges.holdStep}
                                onChange={(v) =>
                                    setInferRanges((s) => ({
                                        ...s,
                                        holdStep: v,
                                ...
                                info="Bước nhảy của hold. Step nhỏ hơn cho nhiều phương án hơn nhưng chạy lâu hơn."
                            />
                            <InputSmall
                                label="K neighbors"
                                value={inferMemory.k}
                                onChange={(v) =>
                                    setInferMemory((s) => ({ ...s, k: v }))
                                }
                                info="Số event quá khứ giống nhất được lấy làm bộ nhớ để vote regime và ước lượng expected return/risk."
                            />
                            <InputSmall
                                label="Min history events"
                                value={inferMemory.minHistoryEvents}
                                onChange={(v) =>
                                    setInferMemory((s) => ({
                                        ...s,
                                        minHistoryEvents: v,
                                ...
                                info="Số lượng event lịch sử tối thiểu trước target. Nếu ít hơn ngưỡng này, hệ thống từ chối suy luận để tránh overfit."
                            />
                            <InputSmall
                                label="Regime horizon (h)"
                                value={inferRegime.horizonHours}
                                onChange={(v) =>
                                    setInferRegime((s) => ({
                                        ...s,
                                        horizonHours: v,
                                ...
                                info="Khoảng thời gian nhìn về phía trước để gán nhãn regime lịch sử khi train online memory."
                            />
                            <InputSmall
                                label="Regime ret threshold"
                                value={inferRegime.retThreshold}
                                onChange={(v) =>
                                    setInferRegime((s) => ({
                                        ...s,
                                        retThreshold: v,
                                ...
                                info="Ngưỡng return để coi là quay đầu hoặc sập tiếp. Ví dụ 0.008 tức +/-0.8%."
                            />
                            <InputSmall
                                label="Regime drawdown thres"
                                value={inferRegime.drawdownThreshold}
                                onChange={(v) =>
                                    setInferRegime((s) => ({
                                        ...s,
                                        drawdownThreshold: v,
                                ...
                                info="Ngưỡng drawdown âm dùng để nhận diện trạng thái trend-down mạnh."
                            />
                            <InputSmall
                                label="Risk penalty (MAE)"
                                value={inferScoring.riskPenalty}
                                onChange={(v) =>
                                    setInferScoring((s) => ({
                                        ...s,
                                        riskPenalty: v,
                                ...
                                info="Hệ số phạt rủi ro MAE khi tính raw score. Càng cao thì hệ thống càng né phương án có drawdown lớn."
                            />
                            <InputSmall
                                label="Hold penalty/h"
                                value={inferScoring.holdPenaltyPerHour}
                                onChange={(v) =>
                                    setInferScoring((s) => ({
                                        ...s,
                                        holdPenaltyPerHour: v,
                                    }))
                                }
                                info="Phí phạt theo mỗi giờ giữ lệnh để tránh ưu tiên phương án giữ quá lâu."
                            />
                            <InputSmall
                                label="Delay penalty/h"
                                value={inferScoring.delayPenaltyPerHour}
                                onChange={(v) =>
                                    setInferScoring((s) => ({
                                        ...s,
                                        delayPenaltyPerHour: v,
                                    }))
                                }
                                info="Phí phạt theo mỗi giờ chờ vào lệnh. Dùng để ưu tiên các setup phản ứng nhanh hơn khi lợi thế tương đương."
                            />
                            <InputSmall
                                label="Uncertainty penalty"
                                value={inferScoring.uncertaintyPenalty}
                                onChange={(v) =>
                                    setInferScoring((s) => ({
                                        ...s,
                                        uncertaintyPenalty: v,
                                    }))
                                }
                                info="Phạt độ bất định của expected return (độ lệch chuẩn trên tập neighbors)."
                            />
                            <InputSmall
                                label="Regime bias"
                                value={inferScoring.regimeBias}
                                onChange={(v) =>
                                    setInferScoring((s) => ({
                                        ...s,
                                        regimeBias: v,
                                ...
                                info="Bias cộng thêm khi xác suất mean-revert cao hơn trend-down, giúp ưu tiên action phù hợp regime."
                            />
                            <InputSmall
                                label="Min expected ret"
                                value={inferScoring.minExpectedRet}
                                onChange={(v) =>
                                    setInferScoring((s) => ({
                                        ...s,
                                        minExpectedRet: v,
                                ...
                                info="Ngưỡng lợi nhuận kỳ vọng tối thiểu để giữ lại action. Action thấp hơn ngưỡng sẽ bị loại."
                            />
                        </div>
                        <button
                            onClick={runFutureInfer}
                            className="border border-[var(--border-color)] px-3 py-2 text-[11px] font-semibold"
                        >
                            {futureInferLoading
                                ? "Đang suy luận..."
                        ...
                        <p className="text-[11px] text-[var(--text-muted)]">
                            ...
                            giống nhất bằng cosine similarity (trên feature đã
                            chuẩn hóa), cộng trọng số theo class để ra{" "}
                            <code>
                                raw regime scores -&gt; logits -&gt; softmax
                                probabilities
                            </code>
                            , rồi tính expected metrics từng{" "}
                            <code>Delay/Hold</code> bằng weighted average từ
                            chính nhóm neighbors này.
                        </p>
                        <div className="border border-[var(--border-color)] bg-[var(--bg-main)] p-3">
                            <p className="text-[11px] font-semibold mb-2">
                                Thang đọc nhanh để user căn quyết định
                            </p>
                            <div className="grid gap-2 lg:grid-cols-2 text-[10px] text-[var(--text-muted)]">
                                ...
                                <p>
                                    ...
                                    <code>&gt;=65%</code>, Nghiêng{" "}
                                    <code>55-64%</code>, Cân bằng{" "}
                                    <code>45-54%</code>, Yếu{" "}
                                    <code>&lt;45%</code>.
                                </p>
                                <p>
                                    <b>Expected Ret:</b> Tốt{" "}
                                    <code>&gt;=1.0%</code>, Khá{" "}
                                    <code>0.3-0.99%</code>, Mỏng{" "}
                                    <code>0-0.29%</code>, Âm <code>&lt;0%</code>
                                    .
                                </p>
                                <p>
                                    <b>Expected MAE:</b> càng gần 0 càng an
                                    toàn; ví dụ <code>-0.5%</code> thường an
                                    toàn hơn <code>-2.0%</code>.
                                </p>
                            </div>
                        </div>
                        {futureInferError ? (
                            ...
                            <div className="space-y-3">
                                <div className="grid gap-3 lg:grid-cols-4">
                                    <Metric
                                        ...
                                        value={fmtInt(
                                            futureInferResult?.meta
                                                ?.detectedEvents || 0,
                                        )}
                                        helper="Số event thoả filter q/longShare/z."
                                        info="Tổng event hệ thống phát hiện trong dataset với bộ lọc hiện tại. Đây là tập nguồn để tạo target và lịch sử."
                                    />
                                    <Metric
                                        ...
                                        helper="Số event quá khứ trước target để so sánh."
                                        info="Chỉ đếm các event có thời gian trước target event. Điều này đảm bảo suy luận mô phỏng đúng điều kiện tương lai (không nhìn trước)."
                                    />
                                    <Metric
                                        label="K used"
                                        value={fmtInt(
                                            futureInferResult?.meta?.kUsed || 0,
                                        )}
                                        helper="Số neighbors dùng để vote regime + estimate action."
                                        info="Sau khi tính similarity, hệ thống chỉ giữ top-K event giống nhất để làm bộ nhớ gần nhất cho quyết định."
                                    />
                                    <Metric
                                        label="Target event"
                                        value={fmtTs(
                                            futureInferResult?.targetEvent
                                                ?.timestamp,
                                        )}
                                        helper="Event đang được suy luận như thể nó vừa xảy ra."
                                        info="Đây là event được xem như thời điểm hiện tại t0. Mọi dự đoán delay/hold đều được sinh ra từ snapshot tại mốc này."
                                    />
                                </div>
                                <div className="grid gap-3 lg:grid-cols-4">
                                    <Metric
                                        label="Mean Revert %"
                                        value={`${pctFmt.format((futureInferResult?.regime?.probs?.mean_revert || 0) * 100)}% (${regimeMeanRevertGrade.label})`}
                                        helper={`logit=${Number(futureInferResult?.regime?.logits?.mean_revert || 0).toFixed(3)} | ${regimeMeanRevertGrade.note}`}
                                        info="Xác suất regime quay đầu tăng. Được tính bằng softmax từ logits do top-K neighbors bỏ phiếu trọng số."
                                    />
                                    <Metric
                                        label="Trend Down %"
                                        value={`${pctFmt.format((futureInferResult?.regime?.probs?.trend_down || 0) * 100)}% (${regimeTrendDownGrade.label})`}
                                        helper={`logit=${Number(futureInferResult?.regime?.logits?.trend_down || 0).toFixed(3)} | ${regimeTrendDownGrade.note}`}
                                        info="Xác suất regime tiếp tục giảm sau event hiện tại."
                                    />
                                    <Metric
                                        label="Chop %"
                                        value={`${pctFmt.format((futureInferResult?.regime?.probs?.chop || 0) * 100)}% (${regimeChopGrade.label})`}
                                        helper={`logit=${Number(futureInferResult?.regime?.logits?.chop || 0).toFixed(3)} | ${regimeChopGrade.note}`}
                                        info="Xác suất regime đi ngang/nhiễu, tức lợi thế directional thấp."
                                    />
                                    <Metric
                                        ...
                                        helper="Số cặp delay/hold đủ sample để chấm điểm."
                                        info="Không phải tất cả cặp delay/hold đều hợp lệ; cặp nào sample từ neighbors quá ít sẽ bị loại."
                                    />
                                </div>
                                {futureInferResult?.actions?.chosen ? (
                                    <div className="border border-[var(--border-color)] p-3">
                                        <p className="text-[11px] font-semibold">
                                            Action được chọn và đối chiếu
                                            baseline
                                        </p>
                                        <div className="grid gap-3 lg:grid-cols-4 mt-2">
                                            ...
                                            />
                                            <Metric
                                                label="Expected Ret / MAE"
                                                value={`${pctFmt.format((futureInferResult.actions.chosen.expectedRet || 0) * 100)}% / ${pctFmt.format((futureInferResult.actions.chosen.expectedMae || 0) * 100)}%`}
                                                helper={`ret=${chosenRetGrade.label} (${chosenRetGrade.note}) | uncertainty=${pctFmt.format((futureInferResult.actions.chosen.uncertainty || 0) * 100)}%`}
                                                info="Expected Ret là lợi nhuận kỳ vọng từ top-K neighbors. Expected MAE là drawdown tệ nhất kỳ vọng trong thời gian giữ lệnh."
                                            />
                                            <Metric
                                                label="Realized Chosen (back-check)"
                                                value={
                                                    futureInferResult?.actions
                                                        ?.chosenTradeRealized
                                                        ? `${pctFmt.format((futureInferResult.actions.chosenTradeRealized.ret || 0) * 100)}%`
                                                        : "N/A"
                                                }
                                                helper="Chỉ để audit trên lịch sử, không dùng khi live."
                                            ...
                                            />
                                            <Metric
                                                label="Realized Baseline (1h/8h)"
                                                value={
                                                    futureInferResult?.actions
                                                        ?.baseline?.realized
                                                        ? `${pctFmt.format((futureInferResult.actions.baseline.realized.ret || 0) * 100)}%`
                                                        : "N/A"
                                                }
                                                helper="So với timing cố định."
                                                info="Mốc tham chiếu để biết action động tốt/xấu hơn chiến lược delay-hold cố định."
                                            />
                                        </div>
                                        {futureInferResult?.actions
                                            ?.chosenTradeRealized ? (
                                            <button
                                                ...
                                                onClick={() =>
                                                    setSelectedCustomTrade({
                                                        ts: futureInferResult
                                                            .targetEvent.ts,
                                                        ret: futureInferResult
                                                            .actions
                                                            .chosenTradeRealized
                                                            .ret,
                                                        entryTs:
                                            ...
                                            >
                                                Gắn detail chart theo action đã
                                                chọn
                                            </button>
                                        ) : null}
                                    </div>
                                        ...
                                        <tbody>
                                            {sortedActions.map((a, i) =>
                                                (() => {
                                                    ...
                                                    return (
                                                        <tr
                                                            key={`${a.delayHours}-${a.holdHours}-${i}`}
                                                            className="border-b border-[var(--border-color)]/70"
                                                        >
                                                            <td className="py-2">
                                                                {a.delayHours}h
                                ...
                                <div className="max-h-[320px] overflow-auto thin-scrollbar">
                                    <table className="w-full text-[11px]">
                                        <thead>
                                            <tr className="border-b border-[var(--border-color)] text-left text-[var(--text-muted)]">
                                                <ThWithInfo
                                                    label="Neighbor event"
                                                    info="Event quá khứ có snapshot giống target event theo cosine similarity."
                                                />
                                                <ThWithInfo
                                                    label="Similarity"
                                                    info="Mức giống nhau của vector feature (đã đưa về thang 0-100%)."
                                                />
                                                <ThWithInfo
                                                    label="Weight"
                                                    info="Trọng số dùng trong vote regime và expected metrics; similarity cao sẽ có weight cao hơn."
                                                />
                                                <ThWithInfo
                                                    label="Regime label"
                                                    info="Nhãn regime lịch sử của neighbor (mean_revert, trend_down, chop)."
                                                />
                                                <ThWithInfo
                                                    label="Total"
                                                    info="Tổng liquidation USD tại event neighbor."
                                                />
                                                <ThWithInfo
                                                    label="Long%"
                                                    info="Tỷ lệ long liquidation trên tổng liquidation của neighbor."
                                                />
                                                <ThWithInfo
                                                    label="Z"
                                                    info="Độ bất thường liquidation của neighbor so với cửa sổ lịch sử gần đó."
                                                />
                                    ...
                                    <table className="w-full text-[11px]">
                                        <thead>
                                            <tr className="border-b border-[var(--border-color)] text-left text-[var(--text-muted)]">
                                                <ThWithInfo
                                                    label="Feature"
                                                    info="Tên biến đặc trưng trong snapshot tại thời điểm target event xảy ra."
                                                />
                                                <ThWithInfo
                                                    label="Value @target t0"
                                                    info="Giá trị cụ thể của feature tại t0, chỉ dùng dữ liệu quá khứ/hiện tại (không dùng tương lai)."
                                                />
                                            </tr>
                                        </thead>
                                        <tbody>
            ...
            <div className="px-5">
                <div className="border border-[var(--border-color)] p-3 space-y-3">
                    <p className="text-[12px] font-semibold">
                        Cascade Lab (Có thể chỉnh tham số)
                    </p>
                    <form
                    ...
                    >
                        ...
                        <Input
                            name="q"
                            defaultValue={cascadeOptions?.q ?? 0.95}
                            label="Quantile"
                        />
                        <Input
                            name="minLongShare"
                            defaultValue={cascadeOptions?.minLongShare ?? 0.65}
                            label="Long share tối thiểu"
                        />
                        <Input
                            name="zMin"
                            defaultValue={cascadeOptions?.zMin ?? 1.5}
                            label="Z-Score tối thiểu"
                        />
                        <Input
                            name="zWindowHours"
                            defaultValue={cascadeOptions?.zWindowHours ?? 168}
                            label="Cửa sổ Z (giờ)"
                        />
                        <Input
                            name="entryDelayHours"
                            defaultValue={cascadeOptions?.entryDelayHours ?? 1}
                            label="Trễ vào lệnh (giờ)"
                        />
                        <Input
                            name="holdHours"
                            defaultValue={cascadeOptions?.holdHours ?? 8}
                            label="Thời gian giữ (giờ)"
                        />
                        <button className="border border-[var(--border-color)] px-2 py-2 text-[11px] font-semibold">
                            Tính lại
                        </button>
                    </form>
                    <p className="text-[11px] text-[var(--text-muted)]">
                        Cascade = <code>totalUsd &gt;= quantile</code> +{" "}
                        <code>longShare &gt;= minLongShare</code> +{" "}
                        <code>zScore &gt;= zMin</code>. Win/Lose = vào lệnh sau{" "}
                        <code>entryDelayHours</code>, đóng lệnh sau{" "}
                        <code>holdHours</code>, win nếu return &gt; 0.
                    </p>
                    <div className="text-[11px] text-[var(--text-muted)] space-y-1">
                        <p>
                            <b>Quantile:</b> Ngưỡng độ lớn liquidation. Ví dụ{" "}
                            <code>0.95</code> nghĩa là chỉ lấy các nến thuộc top
                            5% mạnh nhất.
                        </p>
                        <p>
                            <b>Long share tối thiểu:</b> Tỷ trọng long
                            liquidation trong tổng liquidation của nến. Ví dụ{" "}
                            <code>0.65</code> nghĩa là long phải chiếm ít nhất
                            65%.
                        </p>
                        <p>
                            <b>Z-Score tối thiểu:</b> Mức độ bất thường so với
                            lịch sử gần đây. Z càng cao thì cú thanh lý càng đột
                            biến.
                        </p>
                        <p>
                            <b>Cửa sổ Z (giờ):</b> Số giờ dùng để tính trung
                            bình và độ lệch chuẩn cho Z-Score. Cửa sổ lớn = mượt
                            hơn, ít nhiễu hơn.
                        </p>
                        <p>
                            <b>Trễ vào lệnh (giờ):</b> Chờ bao lâu sau nến
                            cascade mới vào lệnh. <code>0</code> = vào ngay,{" "}
                            <code>1</code> = chờ 1 giờ.
                        </p>
                        <p>
                            <b>Thời gian giữ (giờ):</b> Giữ lệnh bao lâu rồi
                            thoát. Win/Lose hiện tại được tính đúng theo thông
                            số này.
                        </p>
                    </div>
                    <div className="grid gap-3 lg:grid-cols-4">
                        <Metric
                            label="Số Cascade"
                            value={fmtInt(cascade.summary.count)}
                            helper="Số sự kiện đạt điều kiện cascade."
                        />
                        <Metric
                            label="Tỷ lệ Win"
                            value={`${pctFmt.format((cascade.summary.winRate || 0) * 100)}%`}
                            helper="Tỷ lệ win theo bộ tham số vào/ra hiện tại."
                        />
                        <Metric
                            label="Lợi nhuận TB/Lệnh"
                            value={`${pctFmt.format((cascade.summary.avgRet || 0) * 100)}%`}
                            helper="Return trung bình mỗi lệnh."
                        />
                        <Metric
                            label="Ngưỡng Cascade"
                            value={fmtUsd(cascade.summary.threshold || 0)}
                            helper="Ngưỡng total liquidation theo quantile."
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="px-5 grid gap-3 lg:grid-cols-4">
                <Metric
                    label="Thời điểm con trỏ"
                    value={fmtTs(hoverIso)}
                    helper="Mốc thời gian bạn đang hover trên chart."
                />
                <Metric
                    label="Event gần nhất"
                    value={near ? fmtTs(near.timestamp) : "N/A"}
                    helper="Event liquidation gần nhất với con trỏ."
                />
                <Metric
                    label="Tổng liquidation event"
            ...
            <div className="px-5 pb-5">
                <div className="border border-[var(--border-color)] p-3">
                    <p className="text-[12px] font-semibold mb-2">
                        Danh sách đầy đủ Cascade Events
                    </p>
                                ...
                                {cascade.events.length === 0 ? (
                                    <tr>
                                        <td
                                            ...
                                            className="py-3 text-[var(--text-muted)]"
                                        >
                                            Không có event theo tham số hiện
                                            tại.
                                        </td>
                                    </tr>
                                ) : null}
                    ...
                    {selectedEventDetail ? (
                        <div className="mt-4 border-t border-[var(--border-color)] pt-3 space-y-3">
                            <div className="flex items-center justify-between gap-2">
                                <p className="text-[12px] font-semibold">
                                    Detail Cascade Event:{" "}
                                    {fmtTs(selectedEventDetail.eventIso)}
                                </p>
                                <button
                                    type="button"
                                    onClick={() => {
                            ...
                            <div className="grid gap-3 lg:grid-cols-4">
                                <Metric
                                    label="Giá BTC tại event"
                                    value={
                                        selectedEventDetail.eventClose
                                            ? `$${selectedEventDetail.eventClose.toLocaleString()}`
                                            : "N/A"
                                    }
                                    helper="Giá close ở nến gần nhất với thời điểm cascade."
                                />
                                <Metric
                                    label="Long/Short/Total"
                                    value={`${fmtUsd(selectedEventDetail.eventLongUsd)} / ${fmtUsd(selectedEventDetail.eventShortUsd)} / ${fmtUsd(selectedEventDetail.eventTotalUsd)}`}
                                    helper="Cấu phần liquidation tại event."
                                />
                                <Metric
                                    label="Long share + Z"
                                    value={`${pctFmt.format(selectedEventDetail.eventLongShare * 100)}% / ${selectedEventDetail.eventZ.toFixed(2)}`}
                                    helper="So với ngưỡng lọc cascade hiện tại."
                                />
                                <Metric
                                    label="Return chiến lược"
                                    value={
                                        selectedEventDetail.eventRet === null
                                            ? "N/A"
                                            : `${pctFmt.format(selectedEventDetail.eventRet * 100)}%`
                                    }
                                    helper={`Ngưỡng total hiện tại: ${fmtUsd(selectedEventDetail.threshold || 0)}.`}
                                />
                            </div>
                            <div className="grid gap-3 lg:grid-cols-3">
                                <Metric
                                    label="Con trỏ đồng bộ"
                                    value={
                                        detailHoverSnapshot
                                            ? fmtTs(detailHoverSnapshot.iso)
                                            : "N/A"
                                    }
                                    helper="Kéo/zoom/rê chuột ở chart nào thì chart còn lại đồng bộ theo cùng mốc."
                                />
                                <Metric
                                    label="Giá BTC tại con trỏ"
                                    value={
                                        Number.isFinite(
                                            detailHoverSnapshot?.close,
                                        )
                                            ? `$${detailHoverSnapshot.close.toLocaleString()}`
                                            : "N/A"
                                    }
                                    helper="Close ở chart giá tại thời điểm con trỏ."
                                />
                                <Metric
                                    label="Liquidation tại con trỏ"
                                    value={
                                        Number.isFinite(
                                            detailHoverSnapshot?.totalUsd,
                                        )
                                            ? `${fmtUsd(detailHoverSnapshot.totalUsd)} (L:${fmtUsd(detailHoverSnapshot.longUsd)} / S:${fmtUsd(detailHoverSnapshot.shortUsd)})`
                                            : "N/A"
                                    }
                                    helper={
                                        detailHoverSnapshot
                                            ? `Long share ${pctFmt.format(detailHoverSnapshot.longSharePct || 0)}%, Z=${(detailHoverSnapshot.z || 0).toFixed(2)}.`
                                            : "Dữ liệu liquidation tại mốc con trỏ."
                                    }
                                />
                            </div>
                                    ...
                                    {selectedEventDetail.entryPoint ? (
                                        <div className="text-[11px] text-[var(--text-muted)] mt-2 space-y-1">
                                            <p>
                                                ...
                                                <b>
                                                    $
                                                    {selectedEventDetail.entryPoint.close?.toLocaleString() ||
                                                        "N/A"}
                                                </b>
                                            </p>
                                            <p>
                                            ...
                                            <p>
                                                ...
                                                | Z:{" "}
                                                <b>
                                                    {selectedEventDetail.entryPoint.z.toFixed(
                                                        2,
                                                    )}
                                                </b>
                                            </p>
                                        ...
                                        <p className="text-[11px] text-[var(--text-muted)] mt-2">
                                            Không có dữ liệu tại mốc vào lệnh.
                                        </p>
                                    )}
                                </div>
                                <div className="border border-[var(--border-color)] bg-[var(--bg-main)] p-3">
                                    <p className="text-[11px] text-[var(--text-muted)]">
                                        Data thời điểm Thoát lệnh
                                    </p>
                                    <p className="text-[13px] font-semibold mt-1">
                                    ...
                                    {selectedEventDetail.exitPoint ? (
                                        <div className="text-[11px] text-[var(--text-muted)] mt-2 space-y-1">
                                            <p>
                                                ...
                                                <b>
                                                    $
                                                    {selectedEventDetail.exitPoint.close?.toLocaleString() ||
                                                        "N/A"}
                                                </b>
                                            </p>
                                            <p>
                                            ...
                                            <p>
                                                ...
                                                | Z:{" "}
                                                <b>
                                                    {selectedEventDetail.exitPoint.z.toFixed(
                                                        2,
                                                    )}
                                                </b>
                                            </p>
                                        ...
                                        <p className="text-[11px] text-[var(--text-muted)] mt-2">
                                            Không có dữ liệu tại mốc thoát lệnh.
                                        </p>
                                    )}
                                </div>
                            </div>
                            <div className="border border-[var(--border-color)] p-3">
                                <p className="text-[11px] font-semibold mb-2">
                                    Chart 1: Giá BTC toàn bộ dataset (highlight
                                    tất cả cascade events)
                                </p>
                                <div
                                    ref={detailPriceRef}
                            ...
                            <div className="border border-[var(--border-color)] p-3">
                                <p className="text-[11px] font-semibold mb-2">
                                    Chart 2: Total liquidation (histogram) +
                                    Long share (%) + Z-Score toàn bộ dataset
                                </p>
                                <div
                                    ref={detailDataRef}
                                    className="w-full h-[260px]"
                        ...
                        <p className="mt-3 text-[11px] text-[var(--text-muted)]">
                            Bấm nút <b>Detail</b> ở cuối mỗi dòng để xem 2 chart
                            chi tiết theo thời điểm event.
                        </p>
                    )}
                    <div className="mt-4 border-t border-[var(--border-color)] pt-3 space-y-2">
                        <p className="text-[12px] font-semibold">
                            Giả lập vốn nếu vào tất cả events
                        </p>
                        <div className="grid gap-3 lg:grid-cols-4 items-end">
                            ...
                            <Metric
                                label="Vốn cuối"
                                value={fmtUsd(capitalProjection.final)}
                                helper="Compound qua toàn bộ events theo thứ tự thời gian."
                            />
                            <Metric
                                label="Lãi/Lỗ tuyệt đối"
                                value={fmtUsd(capitalProjection.pnl)}
                                helper="Vốn cuối - vốn ban đầu."
                            />
                            <Metric
                                label="ROI"
                                value={`${pctFmt.format(capitalProjection.roi * 100)}%`}
                                helper="Tỷ suất sinh lời trên vốn ban đầu."
                            />
                        </div>
                    </div>
                </div>
...
function InfoHint({ text }) {
    ...
    return (
        <span className="relative inline-flex group align-middle ml-1">
            <span className="inline-flex items-center justify-center h-4 w-4 rounded-full border border-[var(--border-color)] text-[10px] font-semibold text-[var(--text-muted)] cursor-help">
                i
            </span>
...
function GradeBadge({ grade }) {
    ...
    if (g.tone === "good")
        cls =
            "border-[color:color-mix(in_oklab,var(--success-text)_55%,white)] text-[var(--success-text)]";
    if (g.tone === "ok")
        cls =
            "border-[color:color-mix(in_oklab,var(--color-accent)_55%,white)] text-[var(--color-accent)]";
    if (g.tone === "bad")
        cls =
            "border-[color:color-mix(in_oklab,var(--danger-text)_55%,white)] text-[var(--danger-text)]";
    return (
        <span
