...
function buildPolymarketSample({ event, timeline, entryDelayHours, nowMs = Date.now() }) {
  ...
  const liveEntryMs = event.closed || hasResolvedData
    ? intendedEntryMs
...
export async function buildPolymarketBtcUpDownResearch({
  ...
  const dailyBacktest = applyBacktest(
    dailySamples.filter((sample) => sample.outcome),
    options,
  );
  const fourHourBacktest = applyBacktest(
  ...
  const [liveDaily, live4hListed] = await Promise.all([
